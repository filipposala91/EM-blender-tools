...
class EMdbListItem(bpy.types.PropertyGroup):
    ...
    sector: prop.StringProperty(
           name="Settore",
           description="A description for this item",
           default="Empty")

    definition: prop.StringProperty(
           name="USM",
           description="A description for this item",
           default="Empty")
    ...
    legante: prop.StringProperty(
           name="Legante USM",
           description="A description for this item",
           default="Empty")

    function: prop.StringProperty(
           name="Funzione statica USM",
           description="A description for this item",
           default="Empty")

    strato: prop.StringProperty(
           name="US",
           description="A description for this item",
           default="Empty")
    
    interpretation: prop.StringProperty(
           name="Interpretazione sintetica US",
           description="A description for this item",
           default="Empty")
...
class EMdb_import_sqlite(bpy.types.Operator):
    ...
    def execute(self, context):
       ...
       if self.db_type == "EMdb":
              nome_tabella = 'Activity'
              
              for row in documento.execute('SELECT * FROM '+nome_tabella):
                     nome_scheda = row[1]
                     scene.emdb_list.add()
                     scene.emdb_list[emdb_list_index].name = nome_scheda
                     scene.emdb_list[emdb_list_index].sector = row[2]
                     scene.emdb_list[emdb_list_index].definition = row[3]
                     scene.emdb_list[emdb_list_index].material = row[4]
                     scene.emdb_list[emdb_list_index].legante = row[5]
                     scene.emdb_list[emdb_list_index].function = row[6]
                     scene.emdb_list[emdb_list_index].strato = row[7]
                     scene.emdb_list[emdb_list_index].interpretation = row[8]
